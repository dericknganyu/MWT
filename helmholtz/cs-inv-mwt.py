...
parser.add_argument('--res', default=100, type = int, help='resolution')
...
################################################################
PATH = "../../../../../../localdata/Derick/stuart_data/Darcy_421/Helmholtz_TrainData=1000_TestData=5000_Resolution=101X101_Domain=[0,1]X[0,1].hdf5"
resultPATH = ""
#Read Datasets
Y_train, X_train, Y_test, X_test = readtoArray(PATH, 1024, 5000, Nx = 100, Ny = 100)
...
if os.path.isfile(resultPATH+'files/inv/lossData_'+TIMESTAMP+'.txt'):
    os.remove(resultPATH+'files/inv/lossData_'+TIMESTAMP+'.txt')  
for ep in range(epochs):
    ...
    file = open(resultPATH+'files/inv/lossData_'+TIMESTAMP+'.txt',"a")
    file.write(str(ep+1)+" "+str(train_l2)+" "+str(test_l2)+"\n")

ModelInfos = "_cs_inv_%03d"%(res)+"~res_"+str(np.round(test_l2,6))+"~RelL2TestError_"+str(ntrain)+"~ntrain_"+str(ntest)+"~ntest_"+str(batch_size)+"~BatchSize_"+str(learning_rate)+\
            "~LR_"+str(wd)+"~Reg_"+str(gamma)+"~gamma_"+str(step_size)+"~Step_"+str(epochs)+"~epochs_"+time.strftime("%Y%m%d-%H%M%S")  
                            
torch.save(model.state_dict(), resultPATH+"files/inv/last_model"+ModelInfos+".pt")
os.rename(resultPATH+'files/inv/lossData_'+TIMESTAMP+'.txt', resultPATH+'files/inv/lossData'+ModelInfos+'.txt')

dataLoss = np.loadtxt(resultPATH+'files/inv/lossData'+ModelInfos+'.txt')
...
plt.savefig(resultPATH+"figures/inv/Error_VS_TrainingStep"+ModelInfos+".png", dpi=500)




model.load_state_dict(torch.load(resultPATH+"files/inv/last_model"+ModelInfos+".pt"))
model.eval()
...
fileName = "Helmholtz_TrainData=1_TestData=1_Resolution=101X101_Domain=[0,1]X[0,1].hdf5"
...
plt.savefig(resultPATH+'figures/inv/compare'+ModelInfos+'.png',dpi=500)
