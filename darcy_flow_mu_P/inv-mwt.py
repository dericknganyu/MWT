...
X_test  = SubSample(X_test , res, res)[:, :-1, :-1]
Y_test  = SubSample(Y_test , res, res)[:, :-1, :-1]
res = res-1
...
x_train = torch.cat([x_train.reshape(ntrain,res,res,1), grid.repeat(ntrain,1,1,1)], dim=3)
x_test = torch.cat([x_test.reshape(ntest,res,res,1), grid.repeat(ntest,1,1,1)], dim=3)
...
plt.xlabel('x')#, fontsize=16, labelpad=15)
plt.ylabel('y')#, fontsize=16, labelpad=15)
plt.title("FDM-UMWT, RelL2Err = "+str(round(myLoss.rel_single(U_MWT, U_FDM).item(), 3)))
